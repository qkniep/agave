...
impl TimerState {
    ...
    fn new(
        ...
        assert!(timeout_config.delta_first_slice <= timeout_config.delta_block);
        let scaled_delta_timeout = Duration::from_secs_f64(
            (timeout_config.delta_timeout.as_secs_f64() * timeout_multiplier).min(MAX_TIMEOUT_SECS),
        );
...
impl Timers {
    ...
    pub(super) fn set_timeouts(
        ...
        let (timer, next_fire) = TimerState::new(slot, timeout_config, now, timeout_multiplier);
...
mod tests {
    ...
    fn timer_state_machine() {
        ...
        let (mut timer_state, next_fire) = TimerState::new(slot, timeout_config, now, 1.0);
    ...
    fn timer_state_with_multiplier() {
        ...
        let (mut timer_state, next_fire) = TimerState::new(slot, timeout_config, now, multiplier);
    ...
    fn timer_state_caps_at_max_timeout() {
        ...
        let timeout_config = TimeoutConfig {
            ...
            delta_first_slice: DELTA_FIRST_SLICE,
            delta_block,
        };
        let (mut timer_state, next_fire) = TimerState::new(100, timeout_config, now, multiplier);
